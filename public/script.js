...
function init() {
  ...
  // window.lastTextAnimation = addText('1', root, false);
}

window.replaceText = function replaceText(text, animated) {
  if (window.lastTextAnimation != undefined) {
    root.scene.remove(window.lastTextAnimation);
  }
  window.lastTextAnimation = addText(text, window.root, animated);
}

function addText(text, root, animate) {
  var textAnimation = createTextAnimation(text);
